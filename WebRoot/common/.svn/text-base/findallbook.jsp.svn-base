		...
		<c:forEach items="${booklist}" var="c" varStatus="vs">
			<tr>
				...
				<td><c:if test="${c.user_id==0}">否</c:if> <c:if
						test="${c.user_id!=0}">是</c:if>
				</td>
				<td>${c.money}</td>
				<td><c:if test="${c.user_id==0}">
				
						<a
							href="${pageContext.request.contextPath}/common/lendbook_${c.booknum}.do">借阅</a>
					</c:if> <c:if test="${c.user_id!=0}">已借阅</c:if></td>
			</tr>
		</c:forEach>
	</table>
</body>
